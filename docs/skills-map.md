...
Which skill/tool builds each phase. Two tiers:
- **Installed here** — already available in this environment (invoke directly).
...
- **Phaser skills — official `phaserjs/phaser` ONLY** (swapped in 2026-07-18; the unofficial Phaser 3
  ...
  removed — wrong major version for a `phaser: ^4.2.1` project). Install syntax is the `repo@skill`
  form; the comma-list `-s a,b,c` form does not install (it only lists):
  - `npx skills add phaserjs/phaser@game-setup-and-config`
  - `npx skills add phaserjs/phaser@game-object-components`
  - `npx skills add phaserjs/phaser@sprites-and-images`
  - `npx skills add phaserjs/phaser@animations`  ← Phase 09 state→animation switching
  - `npx skills add phaserjs/phaser@scenes` · `@loading-assets` · `@cameras` (core/bootstrap)
  - `npx skills add phaserjs/phaser@physics-arcade` (installed for completeness; the sim is custom +
    Phaser-free, so engine physics is not currently used)
  - `npx skills add phaserjs/phaser@tweens` · `@input-keyboard-mouse-touch` · `@text-and-bitmaptext`
  - `npx skills add phaserjs/phaser@v4-new-features` (guards against v3 idioms; Beam renderer/new APIs)
  - `npx skills add phaserjs/phaser@audio-and-sound` (later-phase audio)
...
| 02 Asset pipeline | `phaserjs/phaser@sprites-and-images`, `@loading-assets`, `find-docs` |
...
| 08 Stage runtime | `phaserjs/phaser@game-setup-and-config`, `find-docs` |
| 09 Character art / state→anim | `phaserjs/phaser@animations`, `@game-object-components`, `superpowers:test-driven-development` |
| 10 Playground | `phaserjs/phaser@game-object-components`, `@input-keyboard-mouse-touch`, `frontend-design` |
| 11 Menus/modes/CPU | `frontend-design`, `ui-ux-pro-max`, `motion-design`, `phaserjs/phaser@scenes` |
| 12 Core combat/camera | `phaserjs/phaser@cameras`, `superpowers:test-driven-development`, `find-docs` |
| 13 Guard migration | `superpowers:test-driven-development` |
| 14 HUD skin | `phaserjs/phaser@text-and-bitmaptext`, `@tweens`, `frontend-design`, `motion-design` |
| 15 Specials/meter | `phaserjs/phaser@tweens`, `@audio-and-sound`, `superpowers:test-driven-development` |
| 16 Integration/QA | `playwright-cli`, `superpowers:verification-before-completion` |

## Install status

Global skills store: `~/.agents/skills/`, activated into Claude Code under `~/.claude/skills/`
(lock: `~/.agents/.skill-lock.json`). Skills CLI = `npx skills add|remove|list`.

**Phaser 4 — official `phaserjs/phaser` only (13, as of 2026-07-18 swap):**
- ✅ `game-setup-and-config` · `game-object-components` · `sprites-and-images` (original 3)
- ✅ `animations` · `audio-and-sound` · `scenes` · `loading-assets` · `cameras`
- ✅ `physics-arcade` · `tweens` · `input-keyboard-mouse-touch` · `text-and-bitmaptext` · `v4-new-features`
...
plugin/marketplace — no auto-update; refresh manually with `npx skills update` or re-`add`.
