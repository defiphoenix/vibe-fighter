...
GitHub `roiizchak/vibe-fighter` (**public**) → Vercel project `vibe-fighter` (org
...
- **The repo is public, so a push is also publication.** Code is MIT ([`LICENSE`](LICENSE)); the art and
  audio under `public/` and `concepts/` are proprietary, all rights reserved
  ([`ASSETS-LICENSE.md`](ASSETS-LICENSE.md)). Never commit a secret, and never relicense an asset.
- [`README.md`](README.md) is the public-facing doc — it makes concrete claims about controls, commands
  and the stack. If you change any of those, change it there too; a stale README is a lie to a stranger.
  Its demo video and screenshots are regenerated by `npm run demo` / `npm run demo -- --shots`
  ([`scripts/record-demo.mjs`](scripts/record-demo.mjs)), which plays the real game in a headed browser.
