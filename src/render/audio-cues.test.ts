...
describe("CueDirector — event cues", () => {
  it("splits light from heavy on the ATTACK KEY, not on damage", () => {
    ...
    const d = seeded();
    expect(d.fight([hit("light")], IDLE, NONE, "fight", 100)).toContain("hitLight");
    expect(d.fight([hit("crouchLight")], IDLE, NONE, "fight", 200)).toContain("hitLight");
    expect(d.fight([hit("heavy")], IDLE, NONE, "fight", 300)).toContain("hitHeavy");
    expect(d.fight([hit("airHeavy")], IDLE, NONE, "fight", 400)).toContain("hitHeavy");
  });

  it("treats a special's hit windows as heavy — its activation already fired `super`", () => {
    const d = seeded();
    expect(d.fight([hit("special")], IDLE, NONE, "fight", 100)).toContain("hitHeavy");
    expect(d.fight([{ type: "special", player: 0 }], IDLE, NONE, "fight", 1000)).toContain("super");
  });

  it("maps block, ko and the round events", () => {
    const d = seeded();
    expect(d.fight([{ type: "block", player: 1, data: { attack: "heavy" } }], IDLE, NONE, "fight", 100)).toEqual(["block"]);
    expect(d.fight([{ type: "ko", player: 1 }], IDLE, NONE, "fight", 200)).toEqual(["ko"]);
    expect(d.fight([{ type: "roundStart" }], IDLE, NONE, "fight", 900)).toEqual(["roundStart"]);
    expect(d.fight([{ type: "roundEnd", player: 0 }], IDLE, NONE, "fight", 1500)).toEqual(["roundEnd"]);
  });

  it("collapses a two-fighter trade to ONE cue", () => {
    const d = seeded();
    const both = d.fight([hit("heavy"), hit("heavy")], IDLE, NONE, "fight", 100);
...
describe("CueDirector — cooldowns", () => {
  it("refuses the same cue inside its gap and allows it outside", () => {
    const d = seeded();
    expect(d.fight([hit("light")], IDLE, NONE, "fight", 1000)).toEqual(["hitLight"]);
    expect(d.fight([hit("light")], IDLE, NONE, "fight", 1030)).toEqual([]);   // 30ms < 60ms gap
    expect(d.fight([hit("light")], IDLE, NONE, "fight", 1070)).toEqual(["hitLight"]);
  });
  ...
  it("does not burn a cue's cooldown when the cap dropped it", () => {
    ...
    expect(d.fight([{ type: "block", player: 1, data: { attack: "light" } }], IDLE, NONE, "fight", 1001)).toEqual(["block"]);
  });
});
...
describe("CueDirector — transitions", () => {
  it("fires jump when the sim CONSUMED the jump, and land on the grounded transition", () => {
    const d = seeded();
    expect(d.fight([], [F("jumpRise", false), F("idle")], c(0, { up: true }), "fight", 100)).toEqual(["jump"]);
    expect(d.fight([], [F("jumpFall", false), F("idle")], NONE, "fight", 300)).toEqual([]);  // still airborne
    // Landing has no input to consume, so it stays a transition — and cannot be lost to batching,
    // because being grounded persists.
    expect(d.fight([], [F("idle"), F("idle")], NONE, "fight", 500)).toEqual(["land"]);
    expect(d.fight([], [F("idle"), F("idle")], NONE, "fight", 700)).toEqual([]);
  });

  it("fires whiff from the CONSUMED attack edge, and never for the special", () => {
    const d = seeded();
    expect(d.fight([], IDLE, c(0, { light: true }), "fight", 100)).toEqual(["whiff"]);
    expect(d.fight([], IDLE, NONE, "fight", 500)).toEqual([]);
    expect(d.fight([], IDLE, c(1, { heavy: true }), "fight", 600)).toEqual(["whiff"]);
    // The special's own cue is `super`, fired from the event. A whoosh on top would double it up.
    expect(d.fight([], IDLE, c(0, { special: true }), "fight", 900)).toEqual([]);
  });
  ...
  it("stays silent outside the FIGHT phase, and does not bank a transition across it", () => {
    ...
    const d = new CueDirector();
    d.fight([], [F("ko", false), F("idle")], NONE, "fight", 0);          // baseline: airborne KO
    expect(d.fight([], [F("idle"), F("idle")], NONE, "roundEnd", 100)).toEqual([]);
    expect(d.fight([], [F("idle"), F("idle")], NONE, "intro", 200)).toEqual([]);
    // ...and by the time the fight starts there is no stale transition left to misread.
    expect(d.fight([], [F("idle"), F("idle")], NONE, "fight", 300)).toEqual([]);
  });
  ...
  it("baselines its FIRST observation instead of reading it as a transition", () => {
    ...
    const d = new CueDirector();
    expect(d.fight([], [F("attackHeavy", false), F("jumpRise", false)], NONE, "fight", 0)).toEqual([]);
  });
  ...
  it("still reports event cues outside the fight phase", () => {
    ...
    expect(d.fight([{ type: "roundStart" }], IDLE, NONE, "intro", 100)).toEqual(["roundStart"]);
  });
});
